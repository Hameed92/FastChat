...
from basic_stats import NUM_SERVERS, LOG_ROOT_DIR
...
def get_log_files(max_num_files=None):
    ...
    for d in dates:
        for i in range(NUM_SERVERS):
            name = os.path.expanduser(os.path.join(LOG_ROOT_DIR, f"{d}-conv.json"))
...
def clean_chat_data(log_files, action_type):
    print(f"log_files_len: {len(log_files)}")
    raw_data = []
    for filename in tqdm(log_files, desc="read files"):
        ...
        for line in lines:
            row = json.loads(line)
            if row["type"] in action_type:
                # if row["type"] in ["chat", "upvote", "downvote"]:
                raw_data.append(row)
    ...
    for row in raw_data:
        ...
        for entry in reversed(conversation):
            if entry['role'] == 'user' and last_user_content is None:
